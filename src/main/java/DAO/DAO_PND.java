...
public class DAO_PND {
    ...
    public List<PND> getPND() {
        ...
        try (Connection c1 = DaoFactory.getDataSource().getConnection()) {
            PreparedStatement ps1 = c1.prepareStatement("SELECT t.* FROM STUSPR t ORDER BY STUSPR6 DESC");
            ...
            while (rs1.next()) {
                ...
                try (Connection c2 = DaoFactory.getDataSource().getConnection()) {
                    ...
                    PreparedStatement ps2 = c2.prepareStatement("SELECT t.* FROM US t WHERE US1 = ?");
                    ps2.setString(1, STUSPR1);
                    ResultSet rs2 = ps2.executeQuery();
                    while (rs2.next()) {
                        String US2 = rs2.getString("US2");
                        try (Connection c3 = DaoFactory.getDataSource().getConnection()) {
                            PreparedStatement ps3 = c3.prepareStatement("SELECT t.* FROM UO t WHERE UO1 = ?");
                            ps3.setString(1, US2);
                            ResultSet rs3 = ps3.executeQuery();
                            while (rs3.next()) {
                                String UO3 = rs3.getString("UO3");
                                try (Connection c4 = DaoFactory.getDataSource().getConnection()) {
                                    PreparedStatement ps4 = c4.prepareStatement("SELECT t.* FROM D t WHERE D1 = ?");
                                    ps4.setString(1, UO3);
                ...
                try (Connection c5 = DaoFactory.getDataSource().getConnection()) {
                    ...
                    PreparedStatement ps5 = c5.prepareStatement("SELECT t.* FROM GR t WHERE GR1 = ?");
                ...
                try (Connection c6 = DaoFactory.getDataSource().getConnection()) {
                    ...
                    PreparedStatement ps6 = c6.prepareStatement("SELECT t.* FROM P t WHERE P1 = ?");
                ...
                pndList.add(new PND(abvD, nameD, numberGroup, nameTeacher, numberPND, dateCreation));
            }
